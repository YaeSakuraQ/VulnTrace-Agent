...
class KnowledgeCaptureService:
    def __init__(self, database: Database, knowledge_dir: Path | None = None) -> None:
    ...
    def publish_approved_candidates(self) -> None:
        ...
        lines = [
            "# Learned Candidates",
            "",
            "This file is generated from reviewed learning candidates and is intended for safe probe and verification reuse.",
            "",
        ]
        if not approved:
            lines.append("- No approved learning candidates yet.")
        for item in approved:
            action = item.suggested_action
            lines.extend(
                [
        ...
        self.learned_knowledge_path.write_text("\n".join(lines), encoding="utf-8")
    ...
    def _select_learning_mode(
        ...
        if any(poc.get("module") == "json_rpc" for poc in state.get("pocs", [])):
            return "json_rpc"
        if "aria2" in product_text or "json-rpc" in product_text or "json rpc" in product_text:
            return "json_rpc"
    ...
    def _build_suggested_action(
        ...
        return {
            ...
            "params": {
                ...
                "port": service["port"],
                "scheme": "https" if int(service["port"]) in {443, 8443} else "http",
                "profile": mode if mode in {"php_apache", "json_rpc"} else "generic_web",
            ...
            "rationale": "Reuse the reviewed controlled verification bundle for the matched service fingerprint.",
    ...
    def _build_summary(self, service: dict[str, Any], state: dict[str, Any], mode: str) -> str:
        ...
        return f"Captured a reviewed controlled verification workflow for {target}."
